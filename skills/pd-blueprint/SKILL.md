...
2. 先生成 `product-brief.md`，明确产品目标、目标用户、核心场景、MVP 范围、成功指标、约束、竞品模式、风险和开放问题。
3. 基于 `product-brief.md` 生成 UI 页面蓝图：
   - `ui-flows.md`
   - `ui-pages.md`
   - `ui-screens.md`
   - `ui-components.md`
4. 每个页面必须覆盖 `default`、`loading`、`empty`、`error`、`permission` 状态。
5. 输出摘要必须列出产物目录、已写文件、仍需用户确认的问题，并提示下一步使用 `$pd-figma`。
