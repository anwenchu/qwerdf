...
name: pd-sync
description: >-
  前后端联调工作流，基于前端实现、后端实现、api-contract 和 integration-map 检查接口、字段、状态码、错误结构、权限、分页、状态页、真实依赖接入和 Mock 禁用；发现契约问题时生成 plan-revision，不直接修代码。Use when the user mentions $pd-sync、pd-sync、前后端联调、接口联调、API联调、真实接口联调、Mock禁用检查。
---
...
3. 读取 `tech/task-slices.md`、`tech/api-contract.md`、`tech/dependency-readiness.md`、`tech/integration-map.md`、`tech/frontend/frontend-state-api.md`、`tech/frontend/frontend-implementation-log.md`、`tech/backend/backend-implementation-log.md`。
...
- 必须检查 `tech/dependency-readiness.md` 和 Mock 禁用；存在 `unknown` / `blocked` / `mock-only` 必需依赖、mock client、mock server、stub adapter、fake repository、fake downstream、fixture-only 数据流或 simulator 时，不得写“联调通过”，不得建议进入 `$pd-test`。
- `sync/integration-report.md` 必须记录“真实依赖验证”和“Mock 禁用检查”；没有真实 HTTP client、真实 DB、真实下游调用或真实 API base URL 证据时，联调结论只能是阻断或有残余风险。
...
```text
目标页面 / API
├─ 发现静态页面、纯前端 mock 或 fixture-only 数据流？
│  ├─ 是：记录为 Mock 禁用阻断，不得写真实联调通过
│  └─ 否：进入动态应用验证
└─ 动态应用验证
   ├─ 前后端服务已运行？
   │  ├─ 否：按项目脚本启动或记录无法启动原因
...
3. 检查 `tech/dependency-readiness.md`：本次联调涉及的依赖必须为 `ready` / `not-required`；`unknown` / `blocked` / `mock-only` 写入“真实依赖验证”并阻断联调通过。
4. 生成 `sync/integration-plan.md`，列出联调场景、前端入口、API、数据准备、真实依赖和预期。
5. 启动或连接可用的前后端环境。
6. 检查 path、method、request、response、状态码、错误结构、权限、分页、状态页和真实接口接入。
7. 执行 Mock 禁用检查：检查前端 API client、后端 client/repository、配置开关、日志、请求目标和运行证据是否仍指向 mock/stub/fake/fixture/simulator；发现即归类为 `implementation-defect` 或依赖阻断。
8. 对每个场景记录证据：命令输出、请求 / 响应摘要、真实资源 / 地址、截图、日志位置或未执行原因。
9. 写入 `sync/integration-report.md`，必须包含“真实依赖验证”和“Mock 禁用检查”。
10. 对每个阻断问题做修复路由分类，并写入 `sync/integration-report.md` 的“修复路由”表：
...
11. 若同一个问题无法确定是 `implementation-defect` 还是 `slice-missing`，默认先回 `$pd-plan` 修正任务归属；不得让 `$pd-fe` / `$pd-be` 在无归属 slice 的情况下扩展范围。
