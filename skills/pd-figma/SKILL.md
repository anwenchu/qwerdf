...
- 不得新增 `product-brief.md`、`ui-pages.md`、`ui-screens.md`、`ui-components.md` 中不存在的产品模块、导航、资产类型、页面或业务对象。
- 设计参考只能影响视觉风格、布局密度、组件形态和微交互，不能覆盖产品事实源或带入参考产品的业务语义。

## Gate 1：生成并确认 UI 方向

1. 基于 `product-brief.md` 和页面蓝图生成 `ui-directions.md`。
2. 先写 `产品事实锁定` 和 `参考使用边界`：列出页面范围、支持的业务对象、明确不展示内容，以及参考图中不得带入的模块 / 文案 / 资产。
3. 每个方向都先写短设计计划：颜色、字体、布局、标志性元素、适用受众和页面气质。
4. 必须给出 3 个互相有明显取舍的方向：信息密度、导航结构、视觉风格、组件策略和风险不能只是换名字。
5. 每个方向都必须保持相同的产品范围；方向之间只能改变呈现方式，不能改变产品能力、页面清单或导航语义。
6. 自检每个方向是否像通用模板，是否误带入参考产品的业务模块；如果只是换色、换名或带入范围外内容，先重写再给用户。
7. 给出推荐方向和理由，但必须等待用户明确选择方向 A / B / C，或提供修订后的方向。
8. 用户没有选择时停止，不调用 Figma。
...
- 已对照 `product-brief.md`、`ui-pages.md`、`ui-screens.md`、`ui-components.md` 检查选定 UI 方向，没有新增范围外页面、导航或业务模块。
...
8. 写入 `figma-handoff.md`，记录 Figma URL、frames、产品范围校验、设计系统、状态页、截图 QA 和前端注意事项。
